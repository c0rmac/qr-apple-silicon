...
kernel void init_identity_q(
    ...
    if (row < M && col < M) { // Q is always M x M
        float val = (row == col) ? 1.0f : 0.0f;
        Q[(ulong)batch * M * M + col * M + row] = val;
    }
}
...
kernel void compute_t_matrix(
    ...
    device float* T_batch   = T_global + ((ulong)batch_idx * BLOCK_SIZE * BLOCK_SIZE);
...
kernel void grid_parallel_update(
    ...
    constant uint& block_start [[buffer(3)]],
    constant uint& is_Q_update [[buffer(4)]],
    ...
    uint max_cols = (is_Q_update == 1) ? M : N;
    ...
    // -------------------------------------------------------------------------
    device const float* A_batch = A + ((ulong)batch_idx * M * N);
    device float* Target_Matrix = (is_Q_update == 0) ? A + ((ulong)batch_idx * M * N) : Q + ((ulong)batch_idx * M * M);
    device const float* T_batch = T_global + ((ulong)batch_idx * BLOCK_SIZE * BLOCK_SIZE);
    ...
    for(int k_tile = 0; k_tile < 4; ++k_tile) {
        ...
        // Swapped coordinates and transpose=true to load -T^T
...
kernel void postprocess_haar_fix(
    ...
    uint batch_idx = grid_pos.z;
    device float* A_batch = A + ((ulong)batch_idx * M * N);
    device float* Q_batch = Q + ((ulong)batch_idx * M * M);
    ...
    for (uint k = tid; k < original_K; k += 1024) {
        if (R_batch[k] < 0.0f) {
            // FIXED: Loop over 'c' to flip Column k of Q (since Q is transposed in memory)
            for (uint c = 0; c < original_M; ++c) {
                Q_batch[k + c * M] = -Q_batch[k + c * M];
            }
            for (uint c = k; c < N; ++c) {
    ...
    if (tid == 0) {
        ...
        if (det_Q < 0.0f) {
            ...
            // FIXED: Loop over 'r' to flip the last Column of Q
            for (uint c = 0; c < original_M; ++c) {
                Q_batch[last_col + c * M] = -Q_batch[last_col + c * M];
            }
